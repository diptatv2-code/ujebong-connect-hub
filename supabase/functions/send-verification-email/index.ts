...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // BUG-011: If a session is provided, ensure it belongs to the same user.
    // We allow unauthenticated calls (so users can resend verification after
    // logout — see AuthContext.signIn which signs out unverified users), but
    // when a JWT IS sent we verify the sub matches the requested user_id.
    // This prevents an authenticated user from triggering verification emails
    // for arbitrary other users.
    ...
    if (authHeader?.startsWith("Bearer ")) {
      ...
      const { data: requester } = await requesterClient.auth.getUser();
      if (requester?.user && requester.user.id !== user_id) {
